...
async def generate_chapter_titles(request: StoryTreeRequest):
    ...
    system_template = (
        ...
        "  ...\n"
        "]\n\n"
        "确保输出是有效的JSON数组，不要包含任何额外的解释或说明文字，每章标题不要包含如“第一章”、“第1章”或“《》”等内容。"
    )
...
async def generate_specific_chapter(request: SpecificChapterRequest):
    ...
    system_template = (
        ...
        "4. 确保所有主要角色都有适当的戏份\n"
        "5. 内容需要连贯，形成一个完整的故事线\n"
        "6. 如果当前章节为最后一章，则此章内容最后要有一个引人入胜的结局\n"
        "7. 章节内容大约500字\n\n"
        "请直接返回章节的详细内容文本，不要包含任何额外的解释或说明文字，如“第X章《标题》”等。\n"
    )
...
async def generate_specific_chapter1(request: SpecificChapterRequest):
    ...
    system_template = (
        ...
        "3. 根据章节在故事中的位置微调各章节的叙事节奏\n"
        "4. 确保所有主要角色都有适当的戏份\n"
        "5. 内容需要连贯，形成一个完整的故事线，并保证语言通顺\n"
        "6. 如果当前章节为最后一章，则此章内容最后要有一个引人入胜的结局\n"
        "7. 章节内容大约500字\n\n"
        "请返回完整的章节内容文本，不要包含任何额外的解释或说明文字（如“第X章《标题》”）。"
    )
