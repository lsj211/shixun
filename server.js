...
//         // 创建缓存键（使用大纲和当前章节场景生成一个唯一的缓存键）
//         const cacheKey = `story_ending_${createHash('md5').update(outline.substring(0, 50)).digest('hex')}_${currentChapter}_${currentScene}`;

//         // 检查缓存
//         const cachedContent = await getFromCache(cacheKey);
//         if (cachedContent) {
//             console.log('缓存命中，返回缓存结局');
//             return res.json(cachedContent);
//         }
...
//         // 调用外部生成故事结局的API
//         const response = await fetch('${PYTHON_API_URL}/api/generate-story-ending', {
//             method: 'POST',
//             headers: {
...
//             return res.status(response.status).json({ error: `API响应错误: ${response.status}, ${errorText}` });
//         }
